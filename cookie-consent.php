 ...
 * cookie-notice.js). Здесь запрос превращается в письмо на адрес из
 * CONSENT_MAIL_TO — это и есть доказательство согласия: дата, страница,
 * браузер и адрес посетителя.
 *
...
/* Адрес заказчика. Несколько адресов — через запятую. */
const CONSENT_MAIL_TO = 'nazarova@rks-nr.ru';

/* ВРЕМЕННО, НА ПРОВЕРКУ. Пока здесь стоит адрес, письма идут на него, а не
   ...
   перепроверить бы не вышло. Часовой предел писем при этом работает.
   Перед сдачей оставить здесь пустые кавычки: получателем снова станет
   CONSENT_MAIL_TO, и всё вернётся к обычному поведению. */
const CONSENT_MAIL_TEST_TO = 'lobyak2401@gmail.com';

/* Отправитель. Домен обязан совпадать с доменом сайта, иначе почта
   получателя отправит письмо в спам. */
const CONSENT_MAIL_FROM = 'no-reply@rks-nr.ru';
